# Midscene AutoGLM 系统提示词（备份）

> 来源：[web-infra-dev/midscene](https://github.com/web-infra-dev/midscene) `packages/core/src/ai-model/models/auto-glm/prompt.ts`
> 改编自智谱 [zai-org/Open-AutoGLM](https://github.com/zai-org/Open-AutoGLM)（Apache-2.0）
> 抓取 commit：`784ce3aab8fb6809a41f493a6ee0a3c3106dab28`
> 备份日期：2026-06-05

提示词在源码中是模板字符串，日期由 `getChineseFormattedDate()` / `getMultilingualFormattedDate()` 运行时注入；本文中以 `{DATE}` 占位。

共 4 个变体：`Plan`（规划，全动作集）× `Locate`（纯定位，只 Tap），各分中文 / 英文（Multilingual）。运行时由 `adapter.ts` 选择。

---

## 1. 中文规划版 `getAutoGLMChinesePlanPrompt`
...
## 2. 英文规划版 `getAutoGLMMultilingualPlanPrompt`
...
## 3. 中文定位版 `getAutoGLMChineseLocatePrompt`
...
## 4. 英文定位版 `getAutoGLMMultilingualLocatePrompt`
...
## 与本项目 `agent_config.dart` 的差异

本项目 `_kDefaultSystemPrompt` 基于上面的「中文规划版」改编：

- **启用** midscene 规则 0 禁用的 4 个动作：`Interact` / `Note` / `Call_API` / `Take_over`（本项目已实现其执行逻辑）。
- **删除** 规则 0（动作禁令）、规则 16（游戏自动战斗）。
- 规则 11 仅保留「咸咖啡」一个示例。
- 坐标系、`<think>/<answer>` 格式、`do(...)` 伪代码与原版一致。
