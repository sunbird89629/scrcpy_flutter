...
void main() {
  group('AdbClient Integration Tests', () {
    ...
    test('getVersion returns adb version', () async {
      try {
    ...
    test('devices returns list of connected devices', () async {
      try {
    ...
    test('getVersion fails with wrong adb path', () async {
      ...
      await expectLater(badClient.getVersion(), throwsA(isA<AdbException>()));
    });

    test('devices command parses output correctly with real adb', () async {
      try {
        final devices = await client.getDevices();
        // 验证返回值是字符串列表
        expect(devices, isA<List<String>>());
